#!/bin/bash
# Test script for PosFormer using inverted Supernote image

# Get the directory of this script
DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
POSFORMER_DIR="$DIR/../PosFormer-main"

# Run test on inverted Supernote image
echo "Testing PosFormer on inverted Supernote image..."
docker run --rm \
...
try:
    ...
    # Process a single Supernote image with inversion
    image_path = '/app/assets/20250422_073317_Page_1.png'
    print(f'\\nProcessing image with inversion: {os.path.basename(image_path)}')
    ...
    processed = preprocess_image(
        ...
        invert=True,  # Use inversion to match CROHME format
        normalize=False
    )
    ...
    # Try with another Supernote image too
    image_path = '/app/assets/20250422_073317_Page_2.png'
    if os.path.exists(image_path):
        print(f'\\nProcessing second image with inversion: {os.path.basename(image_path)}')
        ...
        # Fix tensor format for the model
        processed_np = processed.squeeze()
        tensor = torch.from_numpy(processed_np).float() / 255.0
