#!/bin/bash
# Working solution for PosFormer with timeout parameter
...
# Run a single test image with timeout parameter
echo "Running PosFormer recognition with timeout..."
docker run --rm \
...
try:
    ...
    # Process a single image with proper inversion
    ...
    # IMPORTANT: Use invert=True for Supernote images to match CROHME format
    print('Preprocessing with invert=True to match CROHME format...')
    ...
    with torch.no_grad():
        try:
            hypotheses = model.beam_search(
                tensor,
                mask,
                beam_size=5,  # Reduced beam size for speed
                max_len=30,   # Reduced max length for speed
                alpha=1.0,
                early_stopping=True,
                temperature=1.0
            )
    ...
    print('\\nKey insight: PosFormer expects images in CROHME format (white strokes on black background)')
    print('Use invert=True when preprocessing Supernote images (black strokes on white background)')
