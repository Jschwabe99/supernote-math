#!/bin/bash
# Test script for a real handwriting sample

# Get the directory of this script
DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
POSFORMER_DIR="$DIR/../PosFormer-main"

# Run a test on a real handwriting sample
echo "Testing PosFormer on real handwriting sample..."
docker run --rm \
...
try:
    ...
    # Process a real handwriting sample
    sample_img = '/app/assets/20250422_073317_Page_5.png'
    print(f'\\nProcessing handwriting sample: {os.path.basename(sample_img)}')
    ...
    # Run beam search with limited parameters for speed
    ...
    with torch.no_grad():
        try:
            hypotheses = model.beam_search(
                tensor,
                mask,
                beam_size=5,      # Moderate beam size
                max_len=30,       # Moderate max length
                alpha=1.0,
                early_stopping=True,
                temperature=1.0
            )
