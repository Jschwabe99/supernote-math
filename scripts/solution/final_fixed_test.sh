#!/bin/bash
# Final test script with fixed tensor dimensions

# Get the directory of this script
DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
POSFORMER_DIR="$DIR/../PosFormer-main"

# Run fixed test with proper tensor dimensions
echo "Running final test with corrected tensor dimensions..."
docker run --rm \
...
try:
    ...
    # Fix tensor format - [batch, channels, height, width] - 4D tensor
    # First convert to numpy array to make reshape operations more reliable
    processed_np = processed.squeeze()  # Remove the extra dimension
    print(f'Squeezed shape: {processed_np.shape}')
    ...
    with torch.no_grad():
        try:
            # Create dummy logger for any logging the model might do
            class DummyLogger:
                def experiment(self):
                    return None
