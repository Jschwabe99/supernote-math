...
docker run --rm \
  ...
  pip install pandas pyarrow matplotlib tqdm pillow --quiet
    ...
    if os.path.exists(parquet_path):
        try:
            ...
            try:
                ...
                print(\"Could not display sample row\")
...
summary_path = os.path.join(output_dir, f\"crohme_{test_year}_summary.md\")
...
print(f\"Accuracy: {match_count/processed_count*100:.2f}% ({match_count}/{processed_count})\" if processed_count > 0 else \"No samples processed\")
