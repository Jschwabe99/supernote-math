#!/bin/bash
# Script to test enhanced preprocessing with fixed imports
...
def enhanced_preprocessing(image, target_resolution=(256, 1024)):
    ...
    dilated = cv2.dilate(binary, kernel, iterations=2)  # More dilation for thicker strokes
...
def thickened_preprocessing(image, target_resolution=(256, 1024)):
    '''Standard preprocessing but with thicker strokes'''
    # Use standard preprocessing first
    ...
    # Add channel dimension
    return dilated.reshape(target_resolution[0], target_resolution[1], 1)
...
for idx, image_path in enumerate(page_images[:3]):  # Process first 3 images
    ...
    thickened = thickened_preprocessing(img_array).squeeze()
    ...
    axes[0, 1].set_title('Standard Inverted (invert=True)')
    axes[0, 1].axis('off')
    
    # Enhanced
    axes[1, 0].imshow(enhanced, cmap='gray')
    axes[1, 0].set_title('Enhanced Method')
    axes[1, 0].axis('off')
    
    # Thickened
    axes[1, 1].imshow(thickened, cmap='gray')
    axes[1, 1].set_title('Thickened Method')
    axes[1, 1].axis('off')
    ...
    thickened_result = run_inference(thickened.reshape(256, 1024, 1), 'Thickened')
    
    # Print results
    print(f'    - Standard: {standard_result[:70]}...' if len(standard_result) > 70 else f'    - Standard: {standard_result}')
    print(f'    - Inverted: {inverted_result[:70]}...' if len(inverted_result) > 70 else f'    - Inverted: {inverted_result}')
    print(f'    - Enhanced: {enhanced_result[:70]}...' if len(enhanced_result) > 70 else f'    - Enhanced: {enhanced_result}')
    print(f'    - Thickened: {thickened_result[:70]}...' if len(thickened_result) > 70 else f'    - Thickened: {thickened_result}')
    ...
    with open(os.path.join(output_dir, f'{os.path.splitext(basename)[0]}_results.txt'), 'w') as f:
        ...
        f.write(f'Thickened: {thickened_result}\\n\\n')
