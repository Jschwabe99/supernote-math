#!/bin/bash
# Solution for PosFormer with correct inversion settings using Python virtual environment

# Get the directory of this script
DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
POSFORMER_DIR="$DIR/../PosFormer-main"
CHECKPOINT_PATH="$POSFORMER_DIR/lightning_logs/version_0/checkpoints/best.ckpt"
CPU_CHECKPOINT_PATH="$POSFORMER_DIR/lightning_logs/version_0/checkpoints/best_cpu.ckpt"

# Activate the virtual environment
source "$DIR/activate_env.sh"

# Check if checkpoint exists
if [ -f "$CPU_CHECKPOINT_PATH" ]; then
  CHECKPOINT_PATH="$CPU_CHECKPOINT_PATH"
  echo "Using CPU-optimized checkpoint at $CHECKPOINT_PATH"
elif [ -f "$CHECKPOINT_PATH" ]; then
  echo "Using original checkpoint at $CHECKPOINT_PATH"
else
  echo "Error: No checkpoint file found at $CHECKPOINT_PATH or $CPU_CHECKPOINT_PATH"
  exit 1
fi

# Run the PosFormer recognition with Python
echo "Running PosFormer recognition with virtual environment..."
python -c "
import os
import sys
import torch
...
try:
    ...
    image_files = []
    for i in range(1, 10):
        path = f'$DIR/assets/20250422_073317_Page_{i}.png'
        if os.path.exists(path):
            image_files.append(path)
    
    # Add Test_Data image if it exists
    test_data_path = '$DIR/assets/Test_Data_Page_1.png'
